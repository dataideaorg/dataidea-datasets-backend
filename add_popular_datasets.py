"""
Script to add popular datasets to the database
Run with: poetry run python add_popular_datasets.py
"""
...
def create_default_user():
    """Create or get default admin user for datasets"""
    ...
    # If no user exists, try to get any superuser
    try:
...
def add_datasets(user, categories):
    ...
    for data in datasets_data:
        ...
        if Dataset.objects.filter(title=data['title']).exists():
            print(f"- Dataset already exists: {data['title']}")
            continue
        ...
        created_count += 1
        print(f"✓ Added dataset: {data['title']}")

    return created_count

def main():
    print("\n" + "="*60)
    print("Adding Popular Datasets to Database")
    print("="*60 + "\n")
    ...
    # Create default user
    print("Step 2: Creating/getting admin user...")
    user = create_default_user()
    print()

    # Add datasets
    print("Step 3: Adding popular datasets...")
    count = add_datasets(user, categories)
    print()

    print("="*60)
    print(f"✅ Successfully added {count} new datasets!")
    print("="*60)
    print("\nYou can now view them in the Django admin or frontend.")
